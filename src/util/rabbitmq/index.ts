...
export abstract class RabbitMQBase {
  ...
  protected async withChannel<T>(fn: (channel: Channel) => Promise<T>): Promise<T> {
  ...
  protected async publishToQueue(queue: string, payload: Flash): Promise<ExecuteResponse> {
    return this.withChannel(async (channel) => {
      await channel.assertQueue(queue, { durable: true });
      const sent = channel.sendToQueue(queue, Buffer.from(JSON.stringify(payload)), { persistent: true });
      if (!sent) {
        // Aggressive status tracking: if payload has tokenId, set error status
        /*
        if (payload && payload.tokenId) {
          await RedisClient.getInstance().setTokenStatus(payload.tokenId.toString(), TokenStatus.ERROR, `Failed to send message to RabbitMQ queue: ${queue}`);
        }
        */
        throw new Error(`Failed to send message to RabbitMQ queue: ${queue}`);
      }
      return {
        status: "queued",
...
export class RabbitImagePush extends RabbitMQBase {
  ...
  constructor() {
    ...
    if (!this.queue) {
      throw new Error("RABBITMQ_QUEUE is not defined in the environment variables");
    }
  }
